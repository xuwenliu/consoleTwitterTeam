<template>
	<div>
		...
		<NoFile v-show="!listData.length">
            ...
            <span slot="info">该用户暂未申请主材补贴</span>
        </NoFile>
				 	...
				 	<div class="materialSubsidy">
			        	<ul>
			        		<li class="bg0" v-for="(item,index) in listData" :key="index">
			        			<div>
			        				<h1 class="f28 fwb">获得了【{{item.materialName}}】的主材补贴资格</h1>
...
export default {
	...
	methods: {
		/*获取列表数据*/
    	getDataList() {
    		//https://devcustomer.yingwumeijia.com:443/{version}/twitter/team/materialInfo?userId=123&pageNum=1&pageSize=20
    		let api = window.Host.apiHost+`/twitter/team/materialInfo?userId=${this.userId}&pageNum=${this.page}&pageSize=${this.pageSize}`;
